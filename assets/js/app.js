/**
 * SOCO Ukraine — клієнтська логіка вітрини.
 *
...
(function () {
  ...
  (function chat() {
    ...
    const GREETING = {
      from: 'bot',
      text: 'Вітаю! Мене звати Анна, я менеджерка SOCO Ukraine. Підберу матеріали, розрахую вартість і відповім щодо термінів доставки. Чим допомогти?',
    };
    ...
    const REPLIES = [
      ...
      [/(доставк|відправ|нова пошта|укрпошт)/i, 'Надсилаємо Новою Поштою та Укрпоштою по всій Україні. Замовлення до 15:00 вирушають день у день, по Києву є курʼєр. Від 3 000 ₴ доставка безкоштовна.'],
      [/(рахун|оплат|безготів|фоп|тов|реквізит)/i, 'Працюємо з ФОП і юрособами: виставимо рахунок з ПДВ, надішлемо договір і закривальні документи. Напишіть ЄДРПОУ та пошту — підготую рахунок протягом 15 хвилин.'],
      [/(знижк|акці|дешевш|прайс|опт)/i, 'Для клінік діє накопичувальна знижка до 15 % та окремі умови на опт. Надішліть приблизний місячний обсяг — розрахую персональний прайс.'],
    ...
    const answer = (text) => {
      ...
      return 'Дякую за повідомлення! Передаю його менеджеру — відповімо протягом кількох хвилин. Якщо питання термінове, телефонуйте: +380 (44) 501-22-80.';
    };
