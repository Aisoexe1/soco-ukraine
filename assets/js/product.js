...
(function () {
  ...
  /* ======================================================================
     Мета-дані сторінки
     ====================================================================== */
  document.title = `${p.name} — купити в Україні | SOCO Ukraine`;
  $('meta[name="description"]').setAttribute('content', `${p.short} Ціна ${S.money(p.price)}. ${brand.name}. Доставка по Україні за 1–2 дні.`);
  const canonical = $('link[rel="canonical"]');
  if (canonical) canonical.href = `https://soco.ua/product.html?id=${encodeURIComponent(p.id)}`;
      ...
      <article class="rounded-3xl bg-white p-6 ring-1 ring-ink-200/70">
        <div class="flex items-start gap-4">
          ...
          <div class="min-w-0 flex-1">
            ...
            <p class="mt-0.5 text-xs text-ink-500">${S.escapeHtml(r.role)} · ${S.escapeHtml(r.city)}</p>
