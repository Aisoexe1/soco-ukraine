/**
 * SOCO Ukraine — кошик і оформлення замовлення.
 */
(function () {
  'use strict';
  ...
  const PROMOS = {
    SOCO10: { type: 'percent', value: 10, label: 'знижка 10 %' },
    CLINIC15: { type: 'percent', value: 15, label: 'знижка 15 % для клінік' },
    DELIVERY: { type: 'delivery', value: 0, label: 'безкоштовна доставка' },
  };
  ...
  function deliveryCost(subtotal) {
    ...
    if (promo && PROMOS[promo].type === 'delivery') return 0;
  ...
  function render() {
    ...
    const promoOff =
      promo && PROMOS[promo].type === 'percent' ? Math.round((subtotal * PROMOS[promo].value) / 100) : 0;
    const delivery = deliveryCost(subtotal);
    const total = subtotal - promoOff + delivery;
    ...
    $('[data-sum-promo-row]').classList.toggle('hidden', !promoOff);
    $('[data-sum-promo-row]').classList.toggle('flex', !!promoOff);
    $('[data-sum-promo-code]').textContent = promo || '';
    $('[data-sum-promo]').textContent = `−${S.money(promoOff)}`;

    $('[data-sum-delivery]').textContent = delivery ? S.money(delivery) : 'Безкоштовно';
    ...
    const box = $('[data-free-progress]');
    const left = FREE_FROM - subtotal;
    if (left > 0) {
      box.classList.remove('hidden');
      $('[data-free-text]').innerHTML = `Додайте товарів ще на <b class="font-semibold">${S.money(left)}</b> — і доставка стане безкоштовною.`;
      $('[data-free-bar]').style.width = `${Math.min(100, (subtotal / FREE_FROM) * 100)}%`;
    } else {
      box.classList.remove('hidden');
      $('[data-free-text]').innerHTML = '<b class="font-semibold">Доставка безкоштовна.</b> Замовлення від 3 000 ₴ доставляємо коштом магазину.';
      $('[data-free-bar]').style.width = '100%';
    }
  ...
  /* --- Промокод --- */
  $('[data-promo-apply]').addEventListener('click', () => {
    ...
    if (!code) {
      ...
      msg.className = 'mt-2 text-xs text-ink-500';
      return;
    }
    if (PROMOS[code]) {
      promo = code;
      msg.textContent = `Промокод застосовано: ${PROMOS[code].label}`;
      msg.className = 'mt-2 text-xs font-medium text-emerald-600';
      S.toast('Промокод застосовано', 'ok', PROMOS[code].label);
    } else {
      promo = null;
      msg.textContent = 'Такого промокоду не існує або він вичерпаний';
