...
(function () {
  ...
  (function chat() {
    ...
    // назавжди лишає застарілий текст (стару назву компанії тощо).
    if (!state.chat.length || state.chatGreeting !== GREETING.text) {
      state.chat = [GREETING];
      state.chatGreeting = GREETING.text;
      persist();
    }
    ...
    const REPLIES = [
      ...
      [/(доставк|відправ|нова пошта|курʼєр)/i, 'Нова Пошта до відділення — від 50 ₴, 1–3 дні. Курʼєр Нової Пошти по Україні — від 85 ₴, 1–3 дні.'],
      [/(рахун|оплат|фоп|реквізит|передоплат)/i, 'Оплата на картку або реквізити ФОП, готівкою, післяплата. Доставка — тільки по передоплаті, 200 ₴.'],
